...
function Signup({ onClose, switchToLogin }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, backdropFilter: 'blur(12px)' }}
      ...
      <div style={{ background: '#1E1912', padding: '44px 40px', borderRadius: 24, width: 420, maxWidth: '92vw', maxHeight: '92vh', overflowY: 'auto', position: 'relative', border: '1px solid rgba(196,151,59,0.12)', boxShadow: '0 24px 64px rgba(0,0,0,0.5)', animation: 'scaleIn 0.3s ease' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: 4 }}>Join us to explore the finest Kannauj attars</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 18 }}>
            <label style={{ display: 'block', marginBottom: 8, color: 'var(--text-secondary)', fontWeight: 500, fontSize: '0.85rem' }}>Full Name</label>
        ...
        <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
          ...
          <button onClick={switchToLogin} style={{ background: 'none', border: 'none', color: '#D4A853', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, textDecoration: 'underline' }}>
            Sign in
          </button>
        </p>
      </div>
