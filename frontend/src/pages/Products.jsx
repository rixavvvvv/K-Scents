...
function Products() {
  ...
  const cs = {
    ...
    searchRow: { display: 'flex', gap: 16, justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', marginBottom: 32 },
    searchInput: {
      ...
      color: 'var(--text-primary)', outline: 'none', fontFamily: 'var(--font-body)', transition: 'border-color 0.3s',
    },
    catBar: { display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 32 },
    catBtn: (active) => ({
      padding: '10px 22px', fontSize: '0.85rem', border: active ? '1px solid var(--color-accent)' : '1px solid var(--border-color)',
      borderRadius: 50, cursor: 'pointer', transition: 'all 0.3s', fontWeight: 500,
      background: active ? 'rgba(139,105,20,0.15)' : 'rgba(30,25,18,0.5)', color: active ? '#D4A853' : 'var(--text-secondary)',
      fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', gap: 6,
    }),
    toolbar: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, flexWrap: 'wrap', gap: 16 },
    resultCount: { color: 'var(--text-muted)', fontSize: '0.9rem' },
    sortSelect: {
      padding: '10px 16px', borderRadius: 12, background: 'var(--bg-card)', border: '1px solid var(--border-color)',
      color: 'var(--text-primary)', fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'var(--font-body)', outline: 'none',
    },
    ...
    imgWrap: { position: 'relative', overflow: 'hidden', height: 280, background: '#1A1510' },
    ...
    price: { fontFamily: 'var(--font-heading)', fontSize: '1.3rem', fontWeight: 600, color: '#D4A853' },
    addBtn: (added) => ({
      padding: '10px 24px', borderRadius: 50, border: 'none', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600,
      background: added ? 'rgba(76,175,80,0.2)' : 'linear-gradient(135deg, #8B6914, #D4A853)',
      color: added ? '#4CAF50' : '#0D0B08', transition: 'all 0.3s ease',
      ...(added ? { border: '1px solid rgba(76,175,80,0.3)' } : {}),
    }),
    rating: { display: 'flex', alignItems: 'center', gap: 4, marginBottom: 12 },
    star: { color: '#D4A853', fontSize: '0.85rem' },
    badge: { position: 'absolute', top: 16, left: 16, padding: '5px 14px', borderRadius: 50, fontSize: '0.75rem', fontWeight: 600, background: 'rgba(183,110,121,0.9)', color: '#fff', backdropFilter: 'blur(8px)' },
    stockLow: { position: 'absolute', top: 16, right: 16, padding: '5px 14px', borderRadius: 50, fontSize: '0.75rem', fontWeight: 600, background: 'rgba(212,168,83,0.2)', color: '#D4A853', border: '1px solid rgba(212,168,83,0.3)' },
    emptyState: { textAlign: 'center', padding: '80px 20px' },
    loadingGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 28 },
    skeleton: { background: 'var(--bg-card)', borderRadius: 20, overflow: 'hidden', border: '1px solid var(--border-color)' },
    skelImg: { height: 280, background: 'linear-gradient(90deg, #1A1510 25%, #2A2218 50%, #1A1510 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.5s infinite' },
    skelLine: (w) => ({ height: 16, borderRadius: 8, margin: '12px 24px', width: w, background: 'linear-gradient(90deg, #1A1510 25%, #2A2218 50%, #1A1510 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.5s infinite' }),
  };
  ...
  return (
    <div style={cs.page}>
      <div style={cs.container}>
        {/* Header */}
        <div style={cs.header}>
          <p style={{ fontFamily: 'var(--font-accent)', color: '#D4A853', fontSize: '0.85rem', letterSpacing: 4, textTransform: 'uppercase', marginBottom: 12 }}>
            Our Collection
          </p>
        ...
        <div style={cs.searchRow}>
          <div style={{ position: 'relative', width: '100%', maxWidth: 500 }}>
            <svg style={{ position: 'absolute', left: 18, top: '50%', transform: 'translateY(-50%)', opacity: 0.4 }} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#D4A853" strokeWidth="2"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
        ...
        {error && (
          <div style={cs.emptyState}>
            ...
            <button onClick={() => window.location.reload()} style={{ padding: '10px 28px', borderRadius: 50, background: 'linear-gradient(135deg, #8B6914, #D4A853)', color: '#0D0B08', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem' }}>
              Try Again
            </button>
          </div>
        )}
        ...
        {!loading && !error && filteredProducts.length > 0 && (
          <div style={cs.grid}>
            {filteredProducts.map(product => (
              <div key={product._id} style={cs.card}
                onClick={() => navigate(`/products/${product._id}`)}
                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 20px 50px rgba(0,0,0,0.3)'; e.currentTarget.style.borderColor = 'rgba(196,151,59,0.2)'; const img = e.currentTarget.querySelector('img'); if (img) img.style.transform = 'scale(1.08)'; }}
                onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.borderColor = 'var(--border-color)'; const img = e.currentTarget.querySelector('img'); if (img) img.style.transform = 'scale(1)'; }}>
                <div style={cs.imgWrap}>
                  ...
                  {product.stock === 0 && <span style={{ ...cs.badge, background: 'rgba(139,105,20,0.9)' }}>Sold Out</span>}
                  {product.isOnSale && <span style={cs.badge}>Sale</span>}
                </div>
                <div style={cs.cardBody}>
                  {product.category && (
                    <span style={{ fontSize: '0.7rem', color: '#D4A853', textTransform: 'uppercase', letterSpacing: 2, fontWeight: 600, marginBottom: 6, display: 'block' }}>
                      {product.category}
                    </span>
                  )}
